...
def test_clean_cancels_on_no(configured_repo: Path) -> None:
    ...
    (configured_repo / "build").mkdir()
    r = run("clean", cwd=configured_repo, stdin="n\n")
    assert_failure(r)
    assert (configured_repo / "build").exists()

def test_clean_removes_build_on_yes(configured_repo: Path) -> None:
    """Answering 'y' to the clean prompt should remove the build/ directory."""
    (configured_repo / "build").mkdir()
